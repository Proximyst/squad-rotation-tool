...
function LayerListModal({isOpen, onClose}: { isOpen: boolean, onClose: () => void }): ReactNode & JSX.Element {
  return (
...
function NewLayerModal(
  ...
  const setTypeState = (v?: OnChangeValue<Option, false>) => setNewLayerState({
  ...
  const setTagState = (v?: OnChangeValue<TagOption, false>) => setNewLayerState({
    ...
    layer: v ? newLayerState.map!.findLayers({type: newLayerState.type}).find(l => l.tag === v.value) : undefined,
  });

  const mapOptions = Map.sortedMapsByLocalised().map(map => ({label: map.localised, value: map.name}));
  const mapSelect = <Select options={mapOptions}
                            value={!newLayerState.map ? null : ({
                              label: newLayerState.map.localised,
                              value: newLayerState.map.name,
                            })}
                            onChange={setMapState}/>;
  const typeOptions = newLayerState.map?.findLayerTypes()?.map(type => ({label: type.localised, value: type.name}));
  const typeSelect = <Select options={typeOptions}
                             isDisabled={!newLayerState.map}
                             value={!newLayerState.type ? null : ({
  ...
  return (
      <Modal isOpen={isOpen} onClose={augmentedOnClose} size="full" scrollBehavior="inside">
        <ModalOverlay/>
        <ModalContent margin="2vw">
              ...
              <Table>
                ...
                <Tbody>
                  <Tr key="input">
                    ...
                    <Td>{typeSelect}</Td>
                    <Td>{tagSelect}</Td>
                    <Td>{newLayerState.layer?.faction1?.asTag}</Td>
                    <Td>{newLayerState.layer?.faction2?.asTag}</Td>
                  </Tr>
                </Tbody>
              </Table>
            </TableContainer>
      ...
      <>
        ...
        >
          ...
          >
            ...
            <Button size={'sm'} onClick={layerListDisclosure.onOpen} mx="5px">
              ...
              <Table variant="simple">
                ...
                <Tbody>
                  {state.rows.map((row, idx) => (
                      <Tr key={idx}>
                        <Td isNumeric><EditIcon/> <InfoIcon/></Td>
                        <Td isNumeric>{idx + 1}</Td>
                        <Td>{row?.map?.asTag ?? <></>}</Td>
                        <Td>{row?.type?.asTag ?? <></>}</Td>
                        <Td>{row?.tag ? <Tag>{row.tag}</Tag> : <></>}</Td>
                        <Td>{row ? row.faction1.asTag : <></>}</Td>
                        <Td>{row ? row.faction2.asTag : <></>}</Td>
                      </Tr>
                  ))}
        ...
        <LayerListModal isOpen={layerListDisclosure.isOpen} onClose={layerListDisclosure.onClose}/>
        <NewLayerModal state={state} setState={setState}
                       isOpen={newLayerDisclosure.isOpen} onClose={newLayerDisclosure.onClose}/>
      </>
  );
}
